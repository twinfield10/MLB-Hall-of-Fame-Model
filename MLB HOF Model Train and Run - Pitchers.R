...
p_fit_df_train <- p_get_fit_data(p_tmp, final_game_min='1901-01-01', final_game_max='2011-01-01')%>%
  filter(IPouts_1>=10)
p_fit_df_test <- p_get_fit_data(p_tmp, final_game_min='2011-01-01', final_game_max='2021-31-12') %>%
  filter(IPouts_1>=10)
...
p_xgbFit1 <- train(p_xx, p_yy, method = "xgbLinear", trControl = p_cv_train, verbose = FALSE)
stopCluster(p_cl)
...
p_results_df_test %>% 
  ...
  head(25) %>% 
  select(nameLast, nameFirst, predict, inducted, WAA_1, POS, Active) %>% 
  print.data.frame()
...
p_results_df_test %>% 
  ...
  head(25) %>% 
  select(nameLast, nameFirst, WAR, WARPerYR, predict, WAA_1, Age) %>% 
  print.data.frame()

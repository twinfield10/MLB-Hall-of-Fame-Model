...
#Preview Data
P_bb %>% filter(playerID == 'riverma01') %>% arrange(desc(HOFM_Points))  %>% head(3) %>% print.data.frame()
#### Feature Generation ####
p_tmp <- P_bb %>% filter(POS %in% c('SP', 'RP'))

p_fit_df_train <- p_get_fit_data(p_tmp, final_game_min='1901-01-01', final_game_max='2012-01-01')%>%
  filter(IPouts_1>=10 & playerID %notin% VetCom_IDs)

p_fit_df_test <- rbind(
  p_get_fit_data(p_tmp, final_game_min='2012-01-01', final_game_max='2022-31-12') %>%
    filter(IPouts_1>=10),
  p_get_fit_data(p_tmp, final_game_min='1901-01-01', final_game_max='2011-31-12') %>%
...
# Check #
head(p_fit_df_test %>% filter(playerID == 'kimbrcr01'), 1) %>% print.data.frame()
...
write.csv(p_results_df_test, file = "C:\\Users\\TWinfield\\Desktop\\MLB HOF Model\\PitchersResults_HOFModel.csv")
...
P_bb <- append_pen_wins(P_bb)
P_bb <- append_div_wins(P_bb)
...
p_fit_df_train <- p_get_fit_data(p_tmp, final_game_min='1901-01-01', final_game_max='2012-01-01')%>%
  filter(IPouts_1>=10)
p_fit_df_test <- p_get_fit_data(p_tmp, final_game_min='2012-01-01', final_game_max='2022-31-12') %>%
  filter(IPouts_1>=10)

## Add Aggregate Features ##
p_fit_df_train <- p_fit_df_train %>%
  inner_join(p_get_aggregated_fit_df(p_fit_df_train), by='playerID') %>%
  select(-starts_with(c("WSWin_", "MVPWin_", "GG_Win_", "CYWin_", "AllStar_", "C_HOFM_Points")))
p_fit_df_test <- p_fit_df_test %>%
  inner_join(p_get_aggregated_fit_df(p_fit_df_test), by='playerID') %>%
  select(-starts_with(c("WSWin_", "MVPWin_", "GG_Win_", "CYWin_", "AllStar_", "C_HOFM_Points")))

# Check #
head(p_fit_df_test %>% filter(playerID == 'hallaro01'), 1) %>% print.data.frame()
...
p_results_df_test %>% 
  ...
  filter(TookSteroids == 0  & Eligible == 'Y') %>%
  arrange(desc(predict), desc(HOF_Prob), desc(HOFM_Points), -WAA_1) %>% 
  head(25) %>% 
...
p_imp_scores["CYWins", "Overall"]

## Save Results Table As A CSV ##
write.csv(results_df_test, file = "C:\\Users\\TWinfield\\Desktop\\MLB HOF Model\\PitchersResults_HOFModel.csv")
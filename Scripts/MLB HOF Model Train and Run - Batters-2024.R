...
bb %>% filter(playerID == 'ramirma02')

#### Feature Generation ####
tmp <- bb %>% filter(POS %notin% c('SP', 'RP'))
fit_df_train <- get_fit_data(tmp, final_game_min='1901-01-01', final_game_max='2012-01-01') %>%
  filter(PA_1>=250 & playerID %notin% VetCom_IDs & playerID %notin% c('sheffga01', 'ramirma02'))

fit_df_test <- rbind(
  get_fit_data(tmp, final_game_min='2012-01-01', final_game_max='2022-12-31') %>%
  filter(PA_1>=250),
  get_fit_data(tmp, final_game_min='1901-01-01', final_game_max='2012-12-31') %>%
    filter(PA_1>=250 & playerID %in% VetCom_IDs),
  get_fit_data(tmp, final_game_min='1901-01-01', final_game_max='2020-12-31') %>%
    filter(playerID %in% c('sheffga01', 'ramirma02'))
)
...
fit_df_test %>% filter(playerID %in% c('sheffga01', 'ramirma02'))

fit_df_test %>% filter(playerID %in% HOF_Ballot_IDs) %>% select(playerID) %>% pull()
...
# Results
write.csv(results_df_test, file = "MLB_HOFPredictions_Batters_2023.csv")